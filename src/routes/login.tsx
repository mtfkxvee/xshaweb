...
import { getCurrentCustomer, loginCustomer } from "@/lib/erpnext/auth";
...
function Login() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    // Prime the cache with the freshly-logged-in user before navigating —
    // invalidateQueries alone only marks it stale, so /akun's first render
    // would still see the pre-login "logged out" cache and bounce straight
    // back here before the background refetch resolves.
  ...
  return (
    <SiteLayout>
      <div className="mx-auto flex min-h-[70vh] max-w-container-max items-center justify-center px-gutter py-stack-lg">
