...
export function SiteLayout({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen flex-col">
      {/* Scroll-position sentinel for TopNav's shrink-on-scroll shadow — an
          IntersectionObserver target instead of a window scroll listener,
          so the nav doesn't re-render on every scroll frame. */}
      <div id="scroll-sentinel" aria-hidden="true" className="absolute top-0 h-2 w-px" />
